...
static ngx_int_t
ngx_http_mp4_calc_info_tbl(ngx_http_request_t *r, \
                           ngx_http_mp4_ctx_t *ctx, \
                           ngx_http_mp4_file_t *mp4);
static ngx_int_t
ngx_http_mp4_calc_time_point(ngx_http_request_t *r, \
                             ngx_http_mp4_ctx_t *ctx, \
                             ngx_http_mp4filter_conf_t *conf, \
                             ngx_http_mp4_file_t *mp4, \
                             ngx_http_mp4_trak_t *trak);
static int
ngx_http_mp4_get_sample(ngx_http_request_t *r, \
                        ngx_http_mp4_ctx_t *ctx, \
                        ngx_http_mp4_file_t *mp4, \
                        ngx_http_mp4_trak_t *trak, \
                        ngx_mp4_sample_t *sample);
static ngx_int_t
ngx_http_mp4_get_chunk_offset(ngx_http_request_t *r, \
...
static char mp4_content_type[] = "video/mp4";
...
ngx_http_mp4_handler(ngx_http_request_t *r)
{
    u_char                    *last;
    size_t                     root;
    ngx_int_t                  rc, start, end, unlimit = 0;
    ...
    if (r->args.len) {
        ...
        if (ngx_http_arg(r, (u_char *)"unlimit", 7, &value) == NGX_OK) {
            ngx_set_errno(0);
            unlimit = (int)(strtod((char *)value.data, NULL));
            if (ngx_errno != 0) {
                unlimit = 0;
            }
        }
    ...
    if (start >= 0) {
        ...
        if (unlimit) {
            ...
            ngx_http_mp4_ctx_t *ctx = ngx_http_get_module_ctx(r, ngx_http_mp4_module);
            if (ctx == NULL) {
                ctx = (ngx_http_mp4_ctx_t *)ngx_pcalloc(r->pool, sizeof(ngx_http_mp4_ctx_t));
                if (ctx == NULL) return NGX_HTTP_INTERNAL_SERVER_ERROR;
                ngx_http_set_ctx(r, ctx, ngx_http_mp4_module);
                if (mp4 != NULL) {
                    ngx_int_t rc = ngx_http_mp4_calc_info_tbl(r, ctx, mp4);
    ...
    r->headers_out.content_type.len = sizeof(mp4_content_type) - 1;
    r->headers_out.content_type.data = (u_char *)mp4_content_type;
    if (mp4) {
                           ...
                           ngx_http_mp4_file_t *mp4)
{
    ...
    if (conf->steplen_sec == 0) return NGX_OK;
    ...
    trak = mp4->trak.elts;
    for (i = 0; i < mp4->trak.nelts; i++) {
        rc = ngx_http_mp4_calc_time_point(r, ctx, conf, mp4, &trak[i]);
        if (rc != NGX_OK) return rc;
    }
                             ...
                             ngx_http_mp4_trak_t *trak)
{
    ...
    double end_timepoint = begoff_sec == 0? steplen_sec: begoff_sec;
    ngx_chain_t *cl;
    while ((rc = ngx_http_mp4_get_sample(r, ctx, mp4, trak, &sample)) == NGX_OK) {
        ...
        send_size = sample.off_in_file - last_offset;
        last_offset = sample.off_in_file;
        if (ctx->begoff_offset == 0 && begoff_sec > 0) {
            ctx->begoff_offset = send_size;
        } else {
            cl = (ngx_chain_t *)ngx_pcalloc(r->pool, sizeof(ngx_chain_t));
            if (cl == NULL) return NGX_HTTP_INTERNAL_SERVER_ERROR;
            cl->buf = (ngx_buf_t *)send_size;
            if (ctx->cl_head == NULL) {
                ctx->cl_head = ctx->cl_tail = cl;
            } else {
                ctx->cl_tail->next = cl;
                ctx->cl_tail = cl;
            }
        }
        end_timepoint += steplen_sec;
    }
    if (rc != NGX_OK) return rc;
    return NGX_OK;
}
...
ngx_http_mp4_get_sample(ngx_http_request_t *r, \
                        ...
                        ngx_mp4_sample_t *sample)
{
    ngx_mp4_trak_ctx_t *trak_ctx = &ctx->trak_ctx;
    if (trak_ctx->stts_entry == NULL) {
        ngx_buf_t *stts_data = trak->out[NGX_HTTP_MP4_STTS_DATA].buf;
        trak_ctx->stts_entry = (ngx_mp4_stts_entry_t *)stts_data->pos;
        trak_ctx->stts_end = (ngx_mp4_stts_entry_t *)stts_data->last;
    }
    if (trak_ctx->stco_entry == NULL && trak_ctx->co64_entry == NULL) {
        ngx_buf_t *co_data = trak->out[NGX_HTTP_MP4_CO64_DATA].buf;
        if (co_data == NULL) {
            co_data = trak->out[NGX_HTTP_MP4_STCO_DATA].buf;
            trak_ctx->stco_entry = (uint32_t *)co_data->pos;
            trak_ctx->stco_end = (uint32_t *)co_data->last;
        } else {
            trak_ctx->co64_entry = (uint64_t *)co_data->pos;
            trak_ctx->co64_end = (uint64_t *)co_data->last;
        }
    }
    if (trak_ctx->stsc_entry == NULL) {
        ngx_buf_t *stsc_data = trak->out[NGX_HTTP_MP4_STSC_DATA].buf;
        trak_ctx->stsc_entry = (ngx_mp4_stsc_entry_t *)stsc_data->pos;
        trak_ctx->stsc_end = (ngx_mp4_stsc_entry_t *)stsc_data->last;
    }
    if (trak_ctx->stsz_uniform_size == 0 && trak_ctx->stsz_entry == NULL) {
        ngx_buf_t *stsz_atom, *stsz_data;
        stsz_atom = trak->out[NGX_HTTP_MP4_STSZ_ATOM].buf;
        ngx_mp4_stsz_atom_t *sa = (ngx_mp4_stsz_atom_t *)stsz_atom->pos;
        trak_ctx->stsz_uniform_size = ngx_mp4_get_32value(sa->uniform_size);
        if (trak_ctx->stsz_uniform_size == 0) {
            stsz_data = trak->out[NGX_HTTP_MP4_STSZ_DATA].buf;
            trak_ctx->stsz_entry = (uint32_t *)stsz_data->pos;
            trak_ctx->stsz_end = (uint32_t *)stsz_data->last;
        }
    }

    /*stts*/
    if (trak_ctx->stts_left_count == 0) {
        uint32_t count, duration;
        if (trak_ctx->stts_entry >= trak_ctx->stts_end) {
            sample->timepoint = 0;
            sample->off_in_file = 0;
            return NGX_OK;
        }
        count = ngx_mp4_get_32value(trak_ctx->stts_entry->count);
        duration = ngx_mp4_get_32value(trak_ctx->stts_entry->duration);
        trak_ctx->stts_entry++;
        trak_ctx->stts_left_count = count;
        trak_ctx->stts_duration = duration;
    }
    trak_ctx->cur_time_point += trak_ctx->stts_duration;
    trak_ctx->stts_left_count--;


    /*stsc*/
    ngx_int_t rc;
    size_t chunk_off = 0;
    uint32_t first_chunk, samples;
    if (trak_ctx->stsc_chunk_no == 0) {
        if (trak_ctx->stsc_entry >= trak_ctx->stsc_end) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, \
                "MP4 format error, stsc empty.");
            return NGX_HTTP_INTERNAL_SERVER_ERROR;
        }
        first_chunk = ngx_mp4_get_32value(trak_ctx->stsc_entry->chunk);
        samples = ngx_mp4_get_32value(trak_ctx->stsc_entry->samples);
        trak_ctx->stsc_entry++;
        trak_ctx->stsc_sample_left_count = samples;
        trak_ctx->stsc_chunk_no = first_chunk;
        trak_ctx->stsc_sample_per_chunk = samples;
        rc = ngx_http_mp4_get_chunk_offset(r, trak_ctx, &chunk_off);
        if (rc != NGX_OK) {
            return rc;
        }
    }
    if (trak_ctx->stsc_sample_left_count == 0) {
        if (trak_ctx->stsc_entry < trak_ctx->stsc_end) {
            first_chunk = ngx_mp4_get_32value(trak_ctx->stsc_entry->chunk);
            samples = ngx_mp4_get_32value(trak_ctx->stsc_entry->samples);
            if (trak_ctx->stsc_chunk_no >= first_chunk) {
                trak_ctx->stsc_chunk_no = first_chunk;
                trak_ctx->stsc_sample_left_count = samples;
                trak_ctx->stsc_sample_per_chunk = samples;
                trak_ctx->stsc_entry++;
            } else {
                trak_ctx->stsc_sample_left_count = trak_ctx->stsc_sample_per_chunk;
                trak_ctx->stsc_chunk_no++;
                rc = ngx_http_mp4_get_chunk_offset(r, trak_ctx, &chunk_off);
                if (rc != NGX_OK) {
                    return rc;
                }
    ...
    if (trak_ctx->stsc_sample_left_count > 0)
        trak_ctx->stsc_sample_left_count--;


    /*stsz*/
    uint32_t off = trak_ctx->stsz_uniform_size;
    if (off == 0) {
        if (trak_ctx->stsz_entry >= trak_ctx->stsz_end) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, \
                "MP4 format error, stsz empty.");
            return NGX_HTTP_INTERNAL_SERVER_ERROR;
        }
        off = ngx_mp4_get_32value(trak_ctx->stsz_entry);
        trak_ctx->stsz_entry++;
    }

    trak_ctx->offset += off;

    sample->timepoint = (double)trak_ctx->cur_time_point / trak->timescale;
    sample->off_in_file = trak_ctx->stco_chunk_off + trak_ctx->offset;
    return NGX_OK;
}
...
ngx_http_mp4_get_chunk_offset(ngx_http_request_t *r, \
                              ...
                              size_t *chunk_off)
{
    if (trak_ctx->stco_entry != NULL) {
        if (trak_ctx->stco_entry >= trak_ctx->stco_end) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, \
                "MP4 format error, stco end.");
            return NGX_HTTP_INTERNAL_SERVER_ERROR;
        }
        uint32_t off = ngx_mp4_get_32value(trak_ctx->stco_entry);
        trak_ctx->stco_chunk_off = off;
        trak_ctx->stco_entry++;
    } else {
        if (trak_ctx->co64_entry >= trak_ctx->co64_end) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, \
                "MP4 format error, co64 end.");
            return NGX_HTTP_INTERNAL_SERVER_ERROR;
        }
        uint64_t off = ngx_mp4_get_64value(trak_ctx->co64_entry);
        trak_ctx->stco_chunk_off = off;
        trak_ctx->co64_entry++;
    }
    trak_ctx->offset = 0;
